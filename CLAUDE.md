...
- **Classic library** (`/classics`, `ClassicLibraryScreen`): 46 public-domain puzzles from the 1924 *Cross Word Puzzle Book* (the first crossword book; Gutenberg #68267, .puz via crosserville.com) shipped as static files in `public/classic-puzzles/` with a **generated** `manifest.json` — never hand-edit it, run `pnpm build:classics` (`scripts/build-classic-manifest.mts`, which imports app TS via `scripts/ts-resolve-hook.mjs`). Each manifest entry carries the puzzle's `puzzleIdentity()` so the screen can show solved/PB badges from local solo stats without parsing files. Four of the book's 50 are deliberately excluded (#21, #34 slurs; #40 Klan-joke pseudonym; #42 swastika grid). Pure helpers in `src/lib/classicLibrary.ts`; the import hub previews the 4 smallest and links to `/classics`. "Play solo" follows the daily-mini path (`handleSoloPuzzleLoaded` → `/solo/play`); "Race friends" follows the daily-race path (`setUrlPuzzle` → `/host-game/name`). `classicLibrary.test.ts` verifies the manifest against the files on disk.
- **NYT bookmarklet origin + return loop.** `Puzzle.origin === "nyt-bookmarklet"` marks a bookmarklet import. It is set from the transfer payload (`origin` field, added Sept 2026) AND stamped by `listenForImportedPuzzle`/`readPuzzleFromClipboard` in `puzzleUrl.ts` because those channels are bookmarklet-only and older installed copies send no field. It travels through `puzzleToTransferFormat` (share/challenge/TV links). Consumers: `PuzzleReadyScreen` reports `puzzle_imported {source: "bookmarklet"}` vs `"url"`; `PuzzleReady` shows the "works every day" hook; Solo/Multiplayer play screens call `recordNytPlay()` on finish (separate `nytStreak` in `SoloStats`, same-day idempotent) and pass `nytHook` to `CompletionModal`; `MenuScreen` shows a desktop-only tile (`isDesktopBrowser()` in `src/lib/platform.ts`) linking to `/install-bookmarklet`. The install page carries its own gtag snippet and fires `bookmarklet_install_intent {method}` on drag/contextmenu/click of the bookmark button — that's the only install signal a browser gives.
- **`/install-bookmarklet` is static HTML, not a React route.** Source: `bookmarklet/install-page.html`. Built by `scripts/build-bookmarklet.mjs` into `public/install-bookmarklet/index.html`, which Vite/Vercel serve directly (taking precedence over the SPA). Reason: PR #36 — runtime-assembled `javascript:` URLs in the React bundle were tripping Google Safe Browsing's deceptive-page heuristic. Edit the HTML template, re-run `pnpm build:bookmarklet`. Do not reintroduce a React version.
...
- `pnpm test` — 400 tests across 28 files
...
- **useClueAnnouncer.test.ts** (8 tests): Initial mount skips announcements, new clue triggers speech, last-cell-in-word attribution, no re-announcement of previously completed clues, unknown-player fallback, 2/5s rate cap, disabled state still tracks completions to prevent later blast, null puzzle is a no-op

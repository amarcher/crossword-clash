...
3. **Codex + TTS** (`Codex`): Codex generates commentary text, then speaks via ElevenLabs TTS. Maintains conversation history client-side. Edge function `narrator-Codex` proxies to Codex API. Requires `ANTHROPIC_API_KEY` env var.

### Settings

- `TTSEngine` (`"browser" | "elevenlabs"`) controls per-clue TTS voice when no narrator is active.
- `NarratorEngine` (`"elevenlabs-agent" | "openai-agent" | "Codex" | null`) controls the AI narrator. `null` = no narrator (legacy per-clue announcements).
...
src/
  ...
  lib/
    narrator/
      ...
      claudeNarrator.ts  # Codex + ElevenLabs TTS backend
...
supabase/
  functions/
    ...
    narrator-Codex/   # Codex API proxy for narrator commentary
